...
int ScanRSD(BYTE* pbSrc, bool fRevOrder)
{
	...
	case 0:
		break;//tll 
	default:
		//return -1;  // avoid memory overflow. changed by whr 20170718
		break;
	}
...
int OIWrite_Spec(const ToaMap* pOI, BYTE* pbBuf)
{
	...
	case 0x4000:	//日期时间--属性2
		...
		if (DT_DATE_TIME_S == *pbBuf++)
		{
			...
			if(IsInvalidTime(tm))
			{// 
				DTRACE(DB_FAPROTO, ("date_time_s value invalid\n"));
				return -1;
			}
	...
	else //其他属性
	{
		...
		else
		{
			...
			else if(pOI->wMode == MAP_SYSDB)
			{
				...
				if (0x44010200 == pOI->dwOA)
				{
					DTRACE(DB_FAPROTO, ("OoProReadAttr: refuse read dwOA:%08x !\n", pOI->dwOA));
					return -1;
				}
...
int OoScanData(BYTE* pbSrc, BYTE* pbFmt, WORD wFmtLen, bool fRevOrder, int iIndex, WORD* pwLen, BYTE* pbType, BYTE** ppFieldFmt, WORD* pwFieldFmtLen)
{
	...
	BYTE* pbFmtEnd = pbFmt + wFmtLen;
	int iSP = 0;	//堆栈指针(两个不同堆栈)
	BYTE bFP = 0;	//格式指针
	BYTE bNum=0, bTmpFP=0xff;
	DWORD bLen, bSrcLen, dwNum;
	BYTE bVFmt, bVFill;
...
int DoObjMethod(WORD wOI, BYTE bMethod, BYTE bOpMode, BYTE* pbPara, int* piParaLen, BYTE* pvAddon, BYTE* pbRes)
{
	...
	*piParaLen = 0;
	...
	if((strlen((const char *)pbPara) - offset >= 11) && (*(pbPara + offset) == 0x01))//带时间标签，需判断时效性
	{
		memset((BYTE*)&tm, 0, sizeof(tm));
		tm.nYear = (*(pbPara+offset+1))*256 +*(pbPara+offset+2);
		tm.nMonth = *(pbPara+offset+3);
		tm.nDay = *(pbPara+offset+4);
		tm.nHour = *(pbPara+offset+5);
		tm.nMinute = *(pbPara+offset+6);
		tm.nSecond = *(pbPara+offset+7);
