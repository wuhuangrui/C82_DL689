...
static int _DrawBitmap_Pal(const U8 * pData, int x0, int y0, int XSize, int YSize, int Bpp, int NumColors) {
  ...
  for (i = 0; i < NumColors; i++) {
    ...
    r = *(pData + 2);
    pData += 4;
    LCD__aConvTable[i] = LCD_Color2Index((b << 16) | (g << 8) | r);
  }
  switch (Bpp) {
    case 1:
...
static int _DrawBitmap_24bpp(const U8 * pData, int x0, int y0, int XSize, int YSize) {
  ...
  for (; (y < YSize) && (y >= 0); y += Step) {
    for (x = 0; x < XSize; x++) {
      ...
      g = *(pColor + 1);
      r = *(pColor + 2);
      LCD_SetPixelIndex(x0 + x, y0 + y, LCD_Color2Index((b << 16) | (g << 8) | r));
    }
    pData += BytesPerLine;
  }
  return 0;
...
int GUI_BMP_Draw(const void * pBMP, int x0, int y0) {
  ...
  int NumColors;
  U8 * pSrc = (U8 *)pBMP;
  Type        = _Read16(&pSrc); /* get type from BITMAPFILEHEADER */
  ...
  Height      = _Read32(&pSrc);
  pSrc += 2;
  BitCount    = _Read16(&pSrc);
  Compression = _Read32(&pSrc);
  pSrc += 12;
  ClrUsed     = _Read32(&pSrc);
  pSrc += 4;
